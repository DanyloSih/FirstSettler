    ...
    public struct DeformMaskJob : IJobParallelFor
    {
        ...
        public void Execute(int index)
        {
            ...
            if (unscaledDistance < HalfBrushSize)
            {
                ...
                VoxelData data = rawData[ChunkVoxelsRect.PointToIndex(pointedChunkData)];
                bool changeData = true;

                if (!ChunkVoxelsRect.IsSurfacePoint(pointedChunkData))
                {
                    CubeData cubeData = new CubeData(Surface).Initialize(rawData, ChunkVoxelsRect, pointedChunkData);
                    changeData = !cubeData.IsInvisible();
                }
                ...
                if (changeData)
                {
                    data.Volume = Mathf.Clamp01(data.Volume + DeformFactor);
                }
